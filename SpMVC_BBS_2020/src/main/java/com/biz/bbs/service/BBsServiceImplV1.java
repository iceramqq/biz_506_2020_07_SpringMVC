...
public class BBsServiceImplV1 implements BBsService{
	...
	protected FileService filService;
	...
	public void insert(BBsVO bbsVO,MultipartFile file) {
		...
		String fileName = filService.fileUp(file);
	...
	public int delete(long long_seq) {
		 ...
		 * 1. seq에 해당하는 VO를 dao 에서 findBySeq() 하고
		...
		if(b_file != null) {
			filService.fileDelete(b_file);
		}
		
		filService.fileDelete(bbsVO.getB_file());
	...
	public List<String> insert(BBsVO bbsVO, MultipartHttpServletRequest files) {
		...
		List<ImageVO> fileNames = filService.filesUp(files);
